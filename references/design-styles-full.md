...
> 用于视觉设计（网页/PPT/PDF/封面/配图/App等）的设计风格库。
> 每种风格提供：哲学内核 + 核心特征 + 提示词DNA（与场景模板组合使用）。

## 读取导航

- **何时读取**：`references/design-styles.md` 的快速索引不够用，需要完整 20 种风格、提示词 DNA、风格×场景适配表或现场生成更细的方向说明时读取。
- **何时不读**：用户已有明确品牌、明确方向、正式样稿基准或只需要小修/深化；此时优先沿用用户输入，不硬套风格库。
- **搜索关键词**：信息建筑派、运动诗学派、极简主义派、实验先锋派、东方哲学派、提示词DNA、最佳路径、AI生成、HTML、混合。
- **目录**：风格×场景×执行路径速查表 → 信息建筑派 01-04 → 运动诗学派 05-08 → 极简主义派 09-12 → 实验先锋派 13-16 → 东方哲学派 17-20 → 提示词使用说明。
- **边界**：不要直接写 “in the style of 某机构/设计师”；只抽取可执行视觉特征。品牌资产、客户行业语境和用户确认方向优先于风格库。

## 风格×场景×执行路径 速查表

| 风格 | 网页 | PPT | PDF | 封面 | AI生成 | 最佳路径 |
|------|:---:|:---:|:---:|:---:|:-----:|---------|
| 01 Pentagram | ★★★ | ★★★ | ★★☆ | ★★★ | ★☆☆ | HTML |
| 02 Stamen Design | ★★☆ | ★★☆ | ★★☆ | ★★☆ | ★★☆ | 混合 |
...
**版本**：v2.1
**更新日期**：2026-02-13
**适用场景**：网页/PPT/PDF/封面/配图/App等视觉设计
