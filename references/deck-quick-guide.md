...
- 结构：普通短 deck 先判断是否需要目录页和章节页；命中客户交付时升级到 `references/slide-decks.md` 处理封底策略。
- 署名：普通短 deck 不维护客户署名规则；命中客户交付时升级到 `references/slide-decks.md` 处理 SimpleUX 出品方署名和例外记录。
