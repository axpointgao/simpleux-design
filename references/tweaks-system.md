...
- 方向探索、评审比较、方案选择或需要用户自行调参时
- 你判断少量有启发性的 tweaks 能显著帮助用户理解取舍时

不要把 Tweaks 当成所有任务的默认配件。正式定稿、小修小补、单一明确方向、低风险页面或客户交付版本可以不加；需要加时优先控制在 2-3 个真正影响判断的选项。
...
### 产品原型
- 布局变体（layout A / B / C）
- 交互反馈强度（subtle/standard/expressive）
- 数据量（mock数据条数 5/20/100）
- 状态（empty/loading/success/error）

### 交互与状态
- 页面状态（empty/loading/success/error）
- 导航结构（tabs/sidebar/topbar）
- 表单复杂度（minimal/standard/advanced）
- 反馈方式（toast/inline/modal）
