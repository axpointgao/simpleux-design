...
- 项目方案、POC 提案和设计提案进入制作阶段时，必须已完成 `references/scenario-workflows.md` 与 `references/scenario-gates-and-qa.md` 中的场景确认、材料整合、逐页稿和逐页表达规格确认。
...
- 场景类型：项目方案、POC 提案或设计提案。
- 章节规划：一级章节、页面数量、目录页和章节页判断。
- 逐页稿：每页标题、主结论、关键证据和讲述顺序。
- 逐页表达规格：页面目标、主结论、证据/输入来源、内容结构、表达方式、视觉主角、图形/截图/样稿状态、业务价值连接、占位替换说明和页面风险。
- 样稿状态：已有正式样稿 / 未提供正式样稿。未提供时只能使用线框示意、结构占位或待替换样稿区，并在页面中标注。
- 业务价值连接：每个关键设计判断应能说明 `设计动作 → 用户/运营行为变化 → 业务价值`；缺少真实数据时只写支撑关系，不伪造量化结果。

未完成上述确认时，不直接批量制作高保真 deck；先回到对应场景 reference 补齐内容、问询或研究 brief。
