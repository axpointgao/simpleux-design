...
常规 HTML deck 优先读 `references/deck-quick-guide.md`。本文件用于项目方案、POC 提案、设计提案的制作阶段，以及正式客户交付、复杂结构判断、署名/封底策略冲突或 quick guide 不足以决策的场景。需要完整踩坑记录、长案例和脚本细节时，再读 `references/deck-case-notes.md`。

## 读取导航

- **何时读取**：正式客户交付、三类方案制作阶段、正文 5 页以上、报告型 deck、复杂目录/章节/封底判断、署名策略冲突、导出失败、可编辑 PPTX 前置判断。
- **何时不读**：普通短 deck、内部草稿和快速演示先用 `deck-quick-guide.md`；具体导出坑点先搜索 `deck-case-notes.md`。
- **搜索关键词**：开工停止门禁、多文件架构、STYLE_CONFIRMATION、目录页、章节页、固定封底页、出品方署名、deck:verify、导出、验证清单。
- **目录**：读取边界 → 场景化方案制作入口 → 能力边界 → 开工停止门禁 → 架构选择 → 批量制作规则 → 设计系统 → Deck 基础规则摘要 → 交付门禁脚本 → 多文件模板 → 单文件注意事项 → 导出 → 验证清单 → 分层 QA。
- **边界**：本文件负责制作和正式交付门禁；项目方案/POC/设计提案的上游内容流程只维护在 `scenario-workflows.md`。
...
- 项目方案、POC 提案和设计提案进入制作阶段时，必须已完成 `references/scenario-workflows.md` 中的上游流程和 `references/scenario-gates-and-qa.md` 中的确认门槛。

## 场景化方案制作入口

三类场景不是新的 HTML 渲染层。本文件只消费上游已经确认的方案内容与逐页表达规格，负责把它制作成 HTML-first deck、PDF 或 PPTX。

制作前必须确认：

- 章节规划：一级章节、页面数量、目录页和章节页判断。
- 逐页表达规格：页面目标、主结论、证据/输入来源、内容结构、表达方式、视觉主角、图形/截图/样稿状态、业务价值连接、占位替换说明和页面风险。
- 样稿状态：已有正式样稿 / 未提供正式样稿。未提供时只能使用线框示意、结构占位或待替换样稿区，并在页面中标注。

未完成上述确认时，不直接批量制作高保真 deck；先回到 `scenario-workflows.md` 或 `scenario-gates-and-qa.md` 补齐内容。
...
正式客户交付、项目方案、POC 提案和设计提案默认使用多文件架构。只有正文很短、用户明确要求单文件、且已接受维护与导出风险时，才使用单文件 `<deck-stage>`。
...
- HTML 是源产物，PDF/PPTX 是派生物。
- 默认多文件架构，正式客户交付和三类方案场景不默认使用单文件。
- 正文 5 页以上、正式汇报或客户交付先做结构计划和 2 页视觉确认稿。
- 客户交付 deck 默认包含 SimpleUX 保密署名和固定封底页，例外必须记录。
- 展示 App/Web/PC/移动端界面时使用真实设备样机壳，除非用户确认临时占位。
- 关系、流程、系统、矩阵、模型、闭环、架构、服务地图等页面先做逻辑图形选型。
- 业务价值页和设计策略页避免只写体验价值，必须连接用户行为、运营行为或业务目标。
...
- Level 1 默认执行：HTML 可打开、控制台无错误、翻页交互、全屏隐藏控制层、脚本硬门禁。
- Level 2 抽样执行：正式客户交付、客户方案 deck、含设备样机/逻辑图形/强视觉页时，检查封面、目录/章节页、1-2 张普通内页、封底和所有高风险页。
- Level 3 全量执行：正文 15 页以上、用户明确要求、脚本失败后修复、视觉风险高或导出 PDF/PPTX 后需要逐页确认时。

场景化 QA：

- 项目方案、POC 提案和设计提案还必须回到 `references/scenario-gates-and-qa.md` 检查场景目标、输入依据、样稿状态、逐页表达规格和业务价值连接。
- 不把全量逐页截图复述作为默认交付说明；只汇报失败项、修复项和必要证据。

读取卫生：

- 不把本文件、`deck-case-notes.md`、`editable-pptx.md`、`device-mockups.md` 和 `logic-graphics.md` 全部作为默认必读组合。
- 搜索规则和模板时默认排除 `node_modules/`、`assets/icon/` 和大媒体文件。
- 需要图标时先查图标名称或目标 SVG，不批量读取图标全集。
- 需要设备样机时先读 `assets/device-mockups/manifest.json` 和目标资产路径。
- 需要 publisher 资源时只确认目标文件是否存在，不读取图片、视频二进制内容。
