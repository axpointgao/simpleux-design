...
- HTML → 可编辑 PPTX 导出 → `references/editable-pptx.md` + `scripts/html2pptx.cjs` + `scripts/export_deck_pptx.mjs`（要求 HTML 按 4 条硬约束写）
...
原因：ESM 模块解析会从脚本所在位置向上找 `node_modules`。如果直接从 skill 目录执行，而依赖装在 deck 项目目录，Node 可能找不到。

**对策**：把脚本复制到 deck 项目目录（例如 `brochure/build-pdf.mjs`），在项目根跑 `npm install playwright pdf-lib`，然后 `node build-pdf.mjs --slides slides --out output/deck.pdf`。或者在 skill 目录安装依赖后，从 `simpleux-design/scripts/` 直接执行。

如果在 `simpleux-design/` 目录内使用自带脚本，可先运行：
...
实际 deck 中，把 `assets/publisher/simpleux-dark.png` 和 `simpleux-light.png` 复制到：
...
.publisher-confidential .name {
  font-weight: 650;
}

.publisher-confidential .divider {
  opacity: 0.55;
}
...
```html
<div class="publisher-confidential publisher-confidential--light-bg">
  <img src="../shared/publisher/simpleux-dark.png" alt="" />
  <span class="name">SimpleUX</span>
...
```html
<div class="publisher-confidential publisher-confidential--dark-bg">
  <img src="../shared/publisher/simpleux-light.png" alt="" />
  <span class="name">SimpleUX</span>
  <span class="divider">|</span>
  <span class="label">CONFIDENTIAL&PROPRIETARY</span>
</div>
...
- 内容页 H1 **48-64px**，默认 56-60px
- 内容页标题容器默认 `max-width: 80%`；允许 `text-wrap: balance` 为视觉均衡提前换行
- 只有右侧有截图、图表、模型图或视觉主角时，标题容器才收窄到约 56%-64%
- Hero / 章节 / 大字观点页标题最多 **120px**
- 幻灯片是给 10 米外看的，字要够大
