...
设计评审默认不调用 Lazyweb。只有用户明确要求真实产品参考、竞品对照、UI 模式分析、参考案例、对标产品或“用真实产品证据评审”时，才进入 Lazyweb 候选。
...
用户确认后，先评审当前产物本身，再用 Lazyweb 结果作为对照证据，并在用户项目目录生成 `.lazyweb/design-improve/.../report.html` 或对应研究类型的 HTML 视觉参考板。Lazyweb 结果只用于提炼模式、验证判断和发现遗漏，不替代品牌资产协议，不照抄截图 UI、文案或专有图形。用户拒绝、未确认或 Lazyweb 不可用时，继续按本指南五维评审，并说明未使用 Lazyweb 真实产品参考。
...
- 情绪是否匹配品牌、受众和场景？
- 深浅底、密度、字体和构图可以根据内容反转或重组，但要服务明确目标，且整体气质不能互相打架。
...
- 字体家族是否统一？（通常不超过2种）
- 边缘对齐是否精确？
- 深浅底下，主文字、弱文本、图表标注、按钮状态是否可读？
- 是否检查过关键视口或截图，避免遮挡、溢出、低对比和破版？
...
- App/Web原型是否具备可发现的交互、状态反馈和清楚微文案？
...
| 数据页 / 报告页 | 功能性、视觉层级 | 细节执行 | 创新性（准确优先） |
